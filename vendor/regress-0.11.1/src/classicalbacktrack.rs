...
// JavaScript-compatible regular expressions can contain nested quantifiers.
// Give ordinary programs work proportional to their input and compiled form,
// while retaining a hard ceiling against catastrophic backtracking.
const BASE_EXECUTION_STEPS: usize = 1_000_000;
const STEPS_PER_INPUT_UNIT: usize = 16;
const STEPS_PER_INSTRUCTION: usize = 64;
const MAX_EXECUTION_STEPS: usize = 40_000_000;
...
impl<'a, Input: InputIndexer> MatchAttempter<'a, Input> {
    pub(crate) fn new(
        ...
        Self {
            ...
            remaining_steps: execution_budget,
            limit_exceeded: false,
        }
    }

    #[inline(always)]
    fn consume_steps(&mut self, steps: usize) -> bool {
    ...
    fn run_scm_loop<Dir: Direction>(
        ...
        let scanned_units = input
            .pos_to_offset(max_pos)
            .abs_diff(input.pos_to_offset(start_pos))
            .saturating_add(1);
        if !self.consume_steps(scanned_units) {
            return None;
        }
    ...
    fn try_backtrack<Dir: Direction>(
        ...
        loop {
            if !self.consume_steps(1) {
                // Fuel exhaustion aborts the entire match attempt. Restore the
                // same backtrack-stack invariant as ordinary exhaustion before
        ...
        'nextinsn: loop {
            ...
            'backtrack: loop {
                ...
                match re.insns.iat(ip) {
                    ...
                    &Insn::BackRef(cg_idx) => {
                        let orig_range = self.s.groups.iat(cg_idx as usize).as_range();
                        if let Some(range) = &orig_range {
                            let compared_units = input
                                .pos_to_offset(range.end)
                                .abs_diff(input.pos_to_offset(range.start))
                                .saturating_add(1);
                            if !self.consume_steps(compared_units) {
                                self.bts.truncate(1);
                                return None;
                            }
                        }
                        ...
                        let matched;
                        if let Some(orig_range) = orig_range {
                            if re.flags.icase {
...
pub(crate) fn execution_budget<Input: InputIndexer>(re: &CompiledRegex, input: &Input) -> usize {
    BASE_EXECUTION_STEPS
        .saturating_add(
            input
                .pos_to_offset(input.right_end())
                .saturating_mul(STEPS_PER_INPUT_UNIT),
        )
        .saturating_add(re.insns.len().saturating_mul(STEPS_PER_INSTRUCTION))
...
impl<Input: InputIndexer> BacktrackExecutor<'_, Input> {
    ...
    fn next_match_with_prefix_search<PrefixSearch: bytesearch::ByteSearcher>(
        ...
        let inp = self.input;
        loop {
            if self.matcher.limit_exceeded {
                return None;
            }
...
impl<Input: InputIndexer> exec::MatchProducer for BacktrackExecutor<'_, Input> {
    ...
    fn match_at(&mut self, pos: Input::Position) -> Option<Match> {
        if self.matcher.limit_exceeded {
            return None;
        }
        let inp = self.input;
        let end = self.matcher.try_at_pos(inp, 0, pos, Forward::new())?;
        Some(self.successful_match(pos, end))
    }

    fn execution_limit_exceeded(&self) -> bool {
        self.matcher.limit_exceeded
    }
}
...
impl<'r, 't> exec::Executor<'r, 't> for BacktrackExecutor<'r, Utf8Input<'t>> {
    ...
    fn new(re: &'r CompiledRegex, text: &'t str) -> Self {
        ...
        let budget = execution_budget(re, &input);
        Self {
            input,
            matcher: MatchAttempter::new(re, input.left_end(), budget),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn fuel_exhaustion_restores_backtrack_stack_invariant() {
        let parsed = crate::parse::try_parse("a".chars().map(u32::from), Default::default())
            .expect("test regex must parse");
        let re = crate::emit::emit(&parsed);
        let input = AsciiInput::new("", false);
        let entry = input.left_end();
        let mut matcher = MatchAttempter::new(&re, entry, 0);
        matcher.push_backtrack(BacktrackInsn::SetPosition { ip: 0, pos: entry });
        let mut ip = 0;
        let mut pos = entry;

        assert!(!matcher.try_backtrack(&input, &mut ip, &mut pos, Forward::new()));
        assert!(matcher.limit_exceeded);
        assert_eq!(matcher.bts.len(), 1);
        assert!(matches!(matcher.bts[0], BacktrackInsn::Exhausted));
    }
}
