...
impl ClassSet {
    ...
    fn node(self, icase: bool, unicode: bool, negate_set: bool) -> ir::Node {
        let codepoints = if icase {
            if unicode {
                unicode::add_icase_code_points(self.codepoints)
            } else {
                unicode::add_legacy_icase_code_points(self.codepoints)
            }
        } else {
            self.codepoints
        };
        ...
        loop {
            match self.peek().map(to_char_sat) {
                ...
                Some(']') => {
                    self.consume(']');
                    if self.flags.icase {
                        result.cps = if self.flags.unicode {
                            unicode::add_icase_code_points(result.cps)
                        } else {
                            unicode::add_legacy_icase_code_points(result.cps)
                        };
                    }
            ...
            _ => {
                if Self::is_class_set_reserved_double_punctuator(cp) {
                    if let Some(next) = self.peek() {
                        if Self::is_class_set_reserved_double_punctuator(next) {
