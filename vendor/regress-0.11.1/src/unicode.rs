...
fn legacy_canonicalize(cu: u32) -> u32 {
    crate::legacy_casefold::canonicalize(cu)
}
...
pub fn add_legacy_icase_code_points(input: CodePointSet) -> CodePointSet {
    const CODE_UNIT_MAX: u32 = 0xffff;
    let mut canonical = CodePointSet::new();
    for interval in input.intervals() {
        if interval.first > CODE_UNIT_MAX {
            continue;
        }
        let first = interval.first.min(CODE_UNIT_MAX);
        let last = interval.last.min(CODE_UNIT_MAX);
        if first > last {
            continue;
        }
        for codepoint in first..=last {
            canonical.add_one(legacy_canonicalize(codepoint));
        }
    }

    let mut output = input;
    let mut range_start = None;
    for codepoint in 0..=CODE_UNIT_MAX {
        let matches = canonical.contains(legacy_canonicalize(codepoint));
        match (range_start, matches) {
            (None, true) => range_start = Some(codepoint),
            (Some(start), false) => {
                output.add(Interval::new(start, codepoint - 1));
                range_start = None;
            }
            _ => {}
        }
    ...
    if let Some(start) = range_start {
        output.add(Interval::new(start, CODE_UNIT_MAX));
    }
    output
}
