//! Pinned ECMAScript legacy Canonicalize table.
//!
//! Generated with the project's pinned reference runtime (Node.js 24.18.0) by
//! applying the non-Unicode Canonicalize algorithm to every UTF-16 code unit.
//! Keeping this table local makes legacy /i behavior deterministic across
//! platforms and independent of Rust's Unicode version.

pub(crate) fn canonicalize(code_unit: u32) -> u32 {
    let Ok(code_unit) = u16::try_from(code_unit) else {
        return code_unit;
    };
    match LEGACY_CANONICALIZATION.binary_search_by_key(&code_unit, |&(from, _)| from) {
        Ok(index) => u32::from(LEGACY_CANONICALIZATION[index].1),
        Err(_) => u32::from(code_unit),
    }
}

#[rustfmt::skip]
const LEGACY_CANONICALIZATION: &[(u16, u16)] = &[
    ...
    (0xA7CA, 0xA7C9), (0xA7CD, 0xA7CC), (0xA7CF, 0xA7CE), (0xA7D1, 0xA7D0),
    (0xA7D3, 0xA7D2), (0xA7D5, 0xA7D4), (0xA7D7, 0xA7D6), (0xA7D9, 0xA7D8),
